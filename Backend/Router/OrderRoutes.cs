...
namespace Backend.Router
{
    public static class OrderRoutes
    {
        public static void MapOrderRoutes(this RouteGroupBuilder group, string conn_str)
        {
            ...
            group.MapGet("/orders/user/{user_id}", async (int user_id) =>
            {
                try
                {
                    ...
                    if (orders == null || !orders.Any())
                        return Results.Ok(new { orders = new List<dynamic>() });

                    return Results.Ok(new { orders = orders });
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error in GET /orders/user/{{user_id}}: {ex}");
                    return Results.Problem("Internal server error: " + ex.Message);
                }
            });

            group.MapGet("/orders/stand/{stand_id}", async (int stand_id) =>
            {
                try
                {
                    using var conn = new MySqlConnection(conn_str);
                    ...
                    if (!orders.Any())
                        return Results.Ok(new { orders = new List<object>() });
                    ...
                    {
                        order = o,
                        items = items.Where(i => i.order_id == o.order_id).ToList()
                    });
                ...
                {
                    Console.WriteLine($"Error GET /orders/stand/{stand_id}: {ex}");
                    return Results.Problem("Internal server error: " + ex.Message);
                }
            });
